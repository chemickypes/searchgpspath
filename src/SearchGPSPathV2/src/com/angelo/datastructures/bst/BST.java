...
public class BST {
    ...
    public VInternalNode removeNode(EventQueue eventqueue, VLinkedNode _toremove) {
    ...
    public ArrayList<VInternalNode> getAInternalNode(){
    	ArrayList<VInternalNode> nodes = new ArrayList<>();
    	Queue<VNode> coda = new LinkedList<>();
    	coda.add(rootnode);
    	while(coda.size()>0){
    		VNode n = coda.poll();
    		if(n.isInternalNode()){
    			VInternalNode in = (VInternalNode) n;
    			nodes.add(in);
    			coda.add(in.getLeft());
    			coda.add(in.getRight());
    		}
    ...
    private String strTreeStructure(VNode node, int depth) {
        if ( node==null ) {
            ...
            boolean gotHa = false;
            if(internalnode.get_halfedge()!= null){
            	gotHa = true;
            }
            return "Node #" + internalnode.id + "(v1=" + internalnode.v1.getID() + ",v2=" + internalnode.v2.getID() + ") (parent=" + (internalnode.parent==null?"null":internalnode.parent.id) + ")"+ " "+ gotHa+" #"+
            internalnode.get_halfedge().getpID()+" :\n"
